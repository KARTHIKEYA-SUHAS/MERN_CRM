...
const getClient = () => {
  ...
  if (!apiKey) {
    throw new ApiError(
      503,
      "Gemini API Key is not configured. Add Gemini API Key to the Backend.",
    );
  }
...
const generateText = async (prompt, temperature = 0.7) => {
  ...
  try {
    ...
    console.log("Gemini Text Error:", err?.message || err);
    throw new ApiError(502, "AI Request Failed. Please Try Again in a moment.");
  }
};

export const generateLeadSummary = async (lead) => {
  const prompt = `You are an expert B2B sales analyst for a CRM called Crita CRM. Analyse the following sales lead and produce a concise assessment. 
    ...
    - Potential deal values: $${lead.value || 0}
...
export const generateEmail = async ({ lead, purpose, tone, sender }) => {
  const prompt = `You are a senior sales rep writing on behalf of ${sender?.name || "our team"}${sender?.company ? ` at ${sender.company}` : ""}
    ...
    Return JSON only with compelling subject line and a complete email body.
    Use line breaks (\\n) in the body. Keep it under 180 words. Sign off as ${
      sender?.name || "The Crita Team"
    }.`;
...
export const generateSalesInsights = async (pipelineStats) => {
  const prompt = `You are a revenue-operations advisor. Given this snapshot of the sales pipeline, identify what is working, what is at risk, and concrete actions to improve conversion.
